 ...
 */
// currently unused, so prefix with _, but keep it around in case it's needed
var _netmask = [
    0x00000000,  // 0
    0x80000000,  // 1
    0xC0000000,  // 2
 ...
 * E.g.:
 * prefixBySubnet["255.255.252.0"] === 22
 */
var prefixBySubnet = {
    '0.0.0.0': 0,
    '128.0.0.0': 1,
...
class Instance {
    ...
    nicsToDevices() {
        ...
        this.nics.forEach(function mapNicToDevice(nic, i) {
            var devName = 'net' + i;
            var ipConfig;   // cloud-init version 1 networking
            var ethConfig;  // cloud-init version 2 networking

            // Set parentName to null here so we can assert on it being
            // a string later.
            var parentName = null;
            ...
            // If we didn't find a valid name we'll blow this assert.
            ...
            if (nic.mac) {
                devices[devName]['hwaddr'] = nic.mac;
            }
            if (nic.mtu) {
                // The LXD API needs a string.
                // When you use `lxd config edit ...` the value will get quoted
                // automatically if you input a non-quoted value.
                // If use the API like we do, and you pass a number value
                // then then it fails.
                // The lxd cli client is papering over the issue for you. Why
                // the API can't accept a number value is beyond me.
                devices[devName]['mtu'] = nic.mtu.toString();
            }

            // If the vlan_id is 0 we need to send it out untagged. If you
            // specify a vlan of 0, Linux will send the packet with the vlan
            // tag with a value of zero, which other systems will likely
            // interpret differently and probably drop the packet.
            if (nic.vlan_id && nic.vlan_id > 0) {
                // As with mtu above, this needs to be a string.
                devices[devName]['vlan'] = nic.vlan_id.toString();
            }

            // Done with the device. Now do the IP config.

            // cloud-init version 1 networking
            ipConfig = {
                type: 'physical',
                name: devName,
                subnets: []
            };
            // cloud-init version 2 networking
            ethConfig = {
                'set-name': devName,
                addresses: []
            };
            ...
            if (nic.mac) {
                ipConfig.mac_address = nic.mac;
                ethConfig.match = { macaddress: nic.mac };
            }

            // This whole section will need to be reworked because at some
            // point we want to support ips and gateways and not just
            // ip, subnet, and gateway. This will be required for IPv6.
            // Speaking of v6, should we set accept-ra=false?
            // Separately, maybe we should assert here if netmask is missing?
            if (nic.ip && nic.netmask) {
                var prefixLen = prefixBySubnet[nic.netmask];
                switch (nic.ip) {
                    case 'dhcp':
                        ipConfig.dhcp4 = true;
                        ...
                        break;
                    default:
                        ipConfig.dhcp4 = false;
                        ipConfig.subnets.push({
                            type: 'static',
                            ipv4: true,
                            control: 'auto',
                            address: nic.ip,
                        ...
                        ethConfig.addresses.push(nic.ip + '/' + prefixLen);
                }
            }

            if (nic.primary && nic.gateway && nic.primary === true) {
                ipConfig.subnets[0].gateway = nic.gateway;
                ethConfig.gateway4 = nic.gateway;
            }

            // Version 2 networking adds resolvers for each interface config.
            // So if we're doing v2 networking we'll get the list of resolvers
            // per nic.
            // See below for handling v1.
            if (nic.resolvers) {
                ethConfig.nameservers = {
                    addresses: nic.resolvers
                };
            }

            switch (cloudInitNetwork.version) {
                case 1:
                    cloudInitNetwork.config = [];
                    cloudInitNetwork.config.push(ipConfig);
                    break;
                case 2:
                    cloudInitNetwork.ethernets = {};
                    cloudInitNetwork.ethernets[devName] = ethConfig;
                    break;
                default:
                    // This is impossible to hit because we're explicitly
                    // setting the version to 2 above. But we'll put an assert
                    // here just in case.
                    assert.ok(
                        cloudInitNetwork.version >= 1 &&
                        cloudInitNetwork.version <= 2,
                        'cloud-init network version must be 1 or 2. actual: ' +
                        cloudInitNetwork.version
                    );
            }
        });

        // Version 1 networking adds resolvers as peer objects to interfaces.
        // So if we're doing v1 networking, we'll get the list of resolvers
        // from the top level vm object.
        // See above for handling v2.
        if (this.resolvers && cloudInitNetwork.version === 1) {
            this.resolvers.forEach(function pushResolvers(r) {
                cloudInitNetwork.config.push({
                    type: 'nameserver',
                    address: r
                });
            });
    ...
    toLXD() {
        ...
        payload.devices.root = {
            path: '/',
            pool: 'lxdpool',
            type: 'disk'
        };
        if (this.quota) {
            payload.devices.root.size = util.format('%dGiB', this.quota);
        }
