...
class Instance {
     ...
     */
    nicsToDevices(nics) {
        assert.optionalArrayOfObject(nics, 'nics');

        var i = 0;  // Interface index number
        var devices = {};
        ...
        nics.forEach(function mapNicToDevice(nic, i) {
            var devName = 'net' + i;
            var ipConfig;
            ...
            devices[devName] = {
                ...
                name: devName,
                nictype: 'macvlan',
                // FIXME - Need to look up physical interface name from nic tag
                parent: 'eno1',
                type: 'nic',
    ...
    toLXD() {
        ...
        var networking = this.nicsToDevices(this.nics);

        payload.devices = networking.devices;
        // Network config needs to be the string `#cloud-config\n` followed by
        // a YAML payload. Lucky for us, YAML is a superset of JSON, so we can
        // just stringify it.
...
Instance.fromLxd = function (opts) {
    ...
    console.error('Raw results from LXC:');
