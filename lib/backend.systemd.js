...
class MachineBackend extends EventEmitter {
    ...
    setProp(propname, val, opts) {
        ...
        try {
            return self.lookupProp(propname).set(val);
        } catch (err) {
    ...
    lookupProp(propname) {
        ...
        if (!prop) {
            throw new VError({name: 'NoSuchPropertyError', info: self},
                'property "%s" does not exist', value);
        }
        return prop;
    }
    ...
    watch(callback) {
        ...
        callback(new machine.NotImplementedError(
...
class BackendProp {
     ...
     * @param {[string]} opt.nspawn_section - The section of a systemd.service
...
class DynPropInteger extends BackendProp {
    ...
    validate(value) {
        assert.strictEqual(value, parseInt(value), 'value is an integer');
...
/**
 * Unsigned 32-bit value stored via dbus
 * @extends {DynPropInteger}
 */
class Uint32Prop extends DynPropInteger {
    constructor(backend, name, sdname, opts) {
        super(backend, name, sdname, opts);
        this.min = 0;
        // XXX new in es7?  tell jslint
        this.max = 2**32 - 1;
    }
}
...
/**
 * Escape non alpha-numeric chracters as _<hex> like systemd does
 * @param {string} str - A string that has not been mangled.
 * @return {string} A mangled string.
 */
function mangle(str) {
    assert.string(str);
    regex = /[^a-zA-Z0-9]/g;
    return str.replace(regex, function esc(x) {
        sprintf('_%02x', x.charCodeAt(0))
    });
}
