...
class Machine {
     ...
     * @param opts {object}
     * @param [opts.need_props] {boolean} - If true, properties are loaded,
    ...
    checkImage(prop, value) {
        ...
        var brand = this.getBrandFromImage(json.manifest);
    ...
    checkNICs(_prop, nics) {
        ...
        var ignored = new Set(['belongs_to_type', 'belongs_to_uuid', 'cn_uuid',
            'created_timestamp', 'modified_timestamp', 'owner_uuid', 'state']);
        var dups = {};
        ...
        for (i in nics) {
            ...
            var ignoredProps = nprops.filter(x => ignored.has(x));
            if (ignoredProps.length > 0) {
                self.log.debug('Ignored nic properties %s', ignoredProps);
            }
            var unhandledProps = nprops.filter(x => !allowed.has(x) &&
                !ignored.has(x));
            if (unhandledProps.length > 0) {
                self.log.error('Unhandled nic properties %s', unhandledProps);
            }
        }
        ...
        function safeParse(cfg, parseCb) {
            ...
            fs.readFile(pathname, function _readConfig(err, data) {
                ...
                for (var newkey in newcfg) {
                    target[newkey] = newcfg[newkey];
                    if (cfg === 'tritonConfig') {
                        // XXX TW: This seems broken - having to set config in
                        // two different places!?
                        self.set(newkey, newcfg[newkey]);
                    }
                }
                parseCb();
            });
